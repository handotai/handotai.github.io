...
- Some great news though,
  ...
  - Here were the steps I took:
    - 1. install esp32 script and flash to microcontroller on arduino IDE  (linked below this)
      ...
      5. run both pi and esp32 powered on (power on esp32 and it will start hosting an AP), and ensure that you have KUAL + KOreader on the kindle, and connect them all to the Offline kindle library wifi
      6. Assuming you have books already on the raspberry pi, run a metadata sorting script (also linked below) to sort the books in series in the metadata of the calibre server
      7. run calibre-server, get the IP output and then connect to it via kindle using <http://192.168.blah.blah:8080/opds>, you will need to connect it via OPDS catalog option and that will be the URL to type in
      8. ???
      9. profit, you get your own offline kindle server!!!
